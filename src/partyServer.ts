...
// then `done` long enough for the results screen, and returns to `open`.
...
import {
  DIFFICULTIES, LEVELS, levelUnlocked, MAX_PARTY
} from './shared/levels'
...
/** How long the verdict stays up before the party is back in the hub. */
  ...
  switch (action) {
    case 'create': {
      ...
      const party: Party = {
        ...
        members: [id], ready: new Set([id]), started: 0, ended: 0, slain: 0, total: 0, won: false
      }
    ...
    case 'start': {
      const party = partyOfMember(id)
      if (!party || party.leader !== id || party.state !== 'open') return
      // Everyone in, or nobody goes: a member still browsing the lobby would be
      // pulled into a fight they did not pick.
      if (party.members.some((m) => !party.ready.has(m))) return
      party.state = 'running'
      ...
      console.log(`[Server] party ${party.id} started level ${party.level + 1} (${DIFFICULTIES[party.diff].name}) with ${party.members.length} hero(es)`)
      break
    }
    default:
      return
...
function update(deltaTime: number) {
  ...
  for (const party of parties.values()) {
    ...
    } else if (party.state === 'done' && elapsed - party.ended >= RESULTS_SECONDS) {
      destroyRunSim(party.id)
      party.state = 'open'
      party.ready = new Set(party.members)
      // Back in the hall on their feet; a fallen party is not still down at the bar.
      for (const m of party.members) restoreHero(m)
      // A cleared level opens the next one for the leader's next pick.
      if (party.won && party.level < LEVELS.length - 1) party.level = clampLevel(party.leader, party.level + 1)
      changed = true
    }
  }
  broadcastAge += dt
...
function broadcast() {
  ...
  sendNet('parties', {
    list: [...parties.values()].map((p) => ({
      ...
      slain: p.slain, total: p.total, won: p.won
    }))
  })
}
